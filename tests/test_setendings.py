...
class TestEndingLocations(unittest.TestCase):
    ...
    *tree* and transform each encountered node. Only the nodes which have
    ...
    def test_assignattr(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'AssignAttr.py'))
    ...
    def test_assignname(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'AssignName.py'))
    ...
    def test_asyncfor(self):
        ...
        """
        expected = [(3, 7, 4, 16)]
        module = self.get_file_as_module(path.join(PATH, 'AsyncFor.py'))
        self.set_and_check(module, astroid.AsyncFor, expected)

    def test_asyncfunctiondef(self):
        expected = [(4, 5, 0, 27)]
        module = self.get_file_as_module(path.join(PATH, 'AsyncFunctionDef.py'))
        self.set_and_check(module, astroid.AsyncFunctionDef, expected)

    def test_asyncwith(self):
        expected = [(2, 3, 4, 12)]
        module = self.get_file_as_module(path.join(PATH, 'AsyncWith.py'))
    ...
    def test_attribute(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Attribute.py'))
        self.set_and_check(module, astroid.Attribute, expected)

    def test_augassign(self):
        expected = [(1, 1, 0, 6)]
        module = self.get_file_as_module(path.join(PATH, 'AugAssign.py'))
        self.set_and_check(module, astroid.AugAssign, expected)

    def test_await(self):
        expected = [(5, 5, 4, 27)]
        module = self.get_file_as_module(path.join(PATH, 'Await.py'))
    ...
    def test_binop(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'BinOp.py'))
        self.set_and_check(module, astroid.BinOp, expected)

    def test_boolop(self):
        expected = [(1, 1, 4, 13), (2, 2, 0, 32), (2, 2, 0, 23)]
        module = self.get_file_as_module(path.join(PATH, 'BoolOp.py'))
        self.set_and_check(module, astroid.BoolOp, expected)

    def test_break(self):
        expected = [(2, 2, 4, 9)]
        module = self.get_file_as_module(path.join(PATH, 'Break.py'))
        self.set_and_check(module, astroid.Break, expected)

    def test_call(self):
        expected = [(1, 2, 0, 9)]
        module = self.get_file_as_module(path.join(PATH, 'Call.py'))
        self.set_and_check(module, astroid.Call, expected)

    def test_classdef(self):
        expected = [(1, 3, 0, 8)]
        module = self.get_file_as_module(path.join(PATH, 'ClassDef.py'))
    ...
    def test_compare(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Compare.py'))
    ...
    def test_comprehension(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Comprehension.py'))
    ...
    def test_const(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Const.py'))
        self.set_and_check(module, astroid.Const, expected)

    def test_continue(self):
        expected = [(2, 2, 4, 12)]
        module = self.get_file_as_module(path.join(PATH, 'Continue.py'))
    ...
    def test_decorators(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Decorators.py'))
    ...
    def test_delattr(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'DelAttr.py'))
    ...
    def test_delete(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Delete.py'))
    ...
    #     """
    #     expected = []
    #     module = self.get_file_as_module(path.join(PATH, 'DictUnpack.py'))
    #     self.set_and_check(module, astroid.DictUnpack, expected)

    def test_ellipsis(self):
        expected = [(1, 1, 0, 3)]
        module = self.get_file_as_module(path.join(PATH, 'Ellipsis.py'))
    ...
    #     """
    #     expected = []
    #     module = self.get_file_as_module(path.join(PATH, 'EmptyNode.py'))
    #     self.set_and_check(module, astroid.EmptyNode, expected)

    def test_excepthandler(self):
        expected = [(3, 4, 0, 8)]
        module = self.get_file_as_module(path.join(PATH, 'ExceptHandler.py'))
    ...
    def test_index(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Index.py'))
    ...
    #     """
    #     expected = []
    #     module = self.get_file_as_module(path.join(PATH, 'Print.py'))
    #     self.set_and_check(module, astroid.Print, expected)

    def test_raise(self):
        expected = [(3, 3, 8, 24), (5, 5, 8, 36)]
        module = self.get_file_as_module(path.join(PATH, 'Raise.py'))
    ...
    def test_slice(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Slice.py'))
        self.set_and_check(module, astroid.Slice, expected)

    def test_starred(self):
        expected = [(1, 1, 0, 2), (4, 4, 6, 8)]
        module = self.get_file_as_module(path.join(PATH, 'Starred.py'))
    ...
    def test_subscript(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Subscript.py'))
        self.set_and_check(module, astroid.Subscript, expected)

    def test_tryexcept(self):
        expected = [(1, 4, 0, 8)]
        module = self.get_file_as_module(path.join(PATH, 'TryExcept.py'))
        self.set_and_check(module, astroid.TryExcept, expected)

    def test_tryfinally(self):
        expected = [(1, 6, 0, 8)]
        module = self.get_file_as_module(path.join(PATH, 'TryFinally.py'))
    ...
    def test_tuple(self):
        ...
        module = self.get_file_as_module(path.join(PATH, 'Tuple.py'))
        self.set_and_check(module, astroid.Tuple, expected)

    def test_unaryop(self):
        expected = [(1, 1, 0, 8), (2, 2, 0, 2), (3, 3, 0, 2), (4, 4, 0, 3)]
        module = self.get_file_as_module(path.join(PATH, 'UnaryOp.py'))
        self.set_and_check(module, astroid.UnaryOp, expected)

    def test_while(self):
        expected = [(1, 5, 0, 13)]
        module = self.get_file_as_module(path.join(PATH, 'While.py'))
        self.set_and_check(module, astroid.While, expected)

    def test_with(self):
        expected = [(1, 3, 0, 19)]
        module = self.get_file_as_module(path.join(PATH, 'With.py'))
        self.set_and_check(module, astroid.With, expected)

    def test_yield(self):
        expected = [(2, 2, 4, 11)]
        module = self.get_file_as_module(path.join(PATH, 'Yield.py'))
        self.set_and_check(module, astroid.Yield, expected)

    def test_yieldfrom(self):
        expected = [(2, 2, 4, 23)]
        module = self.get_file_as_module(path.join(PATH, 'YieldFrom.py'))
