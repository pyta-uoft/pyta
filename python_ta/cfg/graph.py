...
class ControlFlowGraph:
    ...
    def get_blocks(self, exclude_unfeasible=False) -> Generator[CFGBlock, None, None]:
        """Generate a sequence of all blocks in this graph.

        When exclude_unfeasible is True, only generate blocks feasible from start based on edge z3 constraints
        """
        yield from self._get_blocks(self.start, set(), exclude_unfeasible)

    def _get_blocks(
        self, block: CFGBlock, visited: Set[int], exclude_unfeasible
        ...
        for edge in block.successors:
            if not exclude_unfeasible or edge.is_feasible:
                yield from self._get_blocks(edge.target, visited, exclude_unfeasible)

    def get_blocks_postorder(self, exclude_unfeasible=False) -> Generator[CFGBlock, None, None]:
        """Return the sequence of all blocks in this graph in the order of
        a post-order traversal.

        When exclude_unfeasible is True, only generate blocks feasible from start based on edge z3 constraints
        """
        yield from self._get_blocks_postorder(self.start, set(), exclude_unfeasible)

    def _get_blocks_postorder(
        self, block: CFGBlock, visited, exclude_unfeasible
    ) -> Generator[CFGBlock, None, None]:
        if block.id in visited:
            return
        ...
        for succ in block.successors:
            if not exclude_unfeasible or succ.is_feasible:
                yield from self._get_blocks_postorder(succ.target, visited, exclude_unfeasible)
