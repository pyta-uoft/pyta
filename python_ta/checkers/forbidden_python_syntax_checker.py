"""A checker for reporting on the disallowed use of various Python elements.
"""
...
class ForbiddenPythonSyntaxChecker(BaseChecker):
    """A checker class to report on the disallowed use of various Python elements.
    ...
    options = (
        (
            "allowed-python-elements",
            {
                "default": (),
                "type": "csv",
                "metavar": "<allowed-elements>",
                "help": "List of Python elements that are allowed to be used.",
            },
        ),
    )
    ...
    def visit_break(self, node: nodes.Break) -> None:
        """Visit a Break node."""
        if "break" not in self.linter.config.allowed_python_elements:
    ...
    def visit_continue(self, node: nodes.Continue) -> None:
        ...
        if "continue" not in self.linter.config.allowed_python_elements:
    ...
    def visit_comprehension(self, node: nodes.Comprehension) -> None:
        ...
        if "comprehension" not in self.linter.config.allowed_python_elements:
            self.add_message("forbidden-comprehension-usage", node=node)

    @only_required_for_messages("forbidden-for-loop-usage")
    def visit_for(self, node: nodes.For) -> None:
        """Visit a For node."""
        if "for" not in self.linter.config.allowed_python_elements:
